...
{
    public partial class fr_ozellikler : Form
    {
        SQL_Yardimcisi SQL_y = new SQL_Yardimcisi();
        Dilbaz dilbaz = new Dilbaz();
        string tablo_adi;
        string urun_karti_ID;
        string Sql_metni;
        string[] kolon_listesi;
        string[] deger_listesi;

        public fr_ozellikler(string tablo_adi,string urun_karti_ID,string urun_adi)
        {
            InitializeComponent();
            this.tablo_adi = tablo_adi;
            this.urun_karti_ID = urun_karti_ID;
            this.Text = urun_adi;
            ...
            for (int i = 0; i < kolon_listesi.Length; i++)
            {
                ...
                if (kolon_listesi[i].Substring(kolon_listesi[i].Length - 3, 3) == "_ID" )
                {
                    ...
                    if (kolon_listesi[i].ToString() == "urun_karti_ID") c.Hide(); 
            ...
            if (yabanci_tablo.Count > 0)
            {
                ...
                string where_bolumu = " Where urun_karti_ID=" + this.urun_karti_ID;
                this.Sql_metni = select_bolumu + From_bolumu+ where_bolumu;
            }
            else
            {
        ...
        private void data_Grid_MouseDoubleClick(object sender, MouseEventArgs e)
        {
            string filtre_ID = data_Grid.SelectedRows[0].Cells["ID"].Value.ToString();
            string adi ="fr_"+ data_Grid.SelectedRows[0].Cells["ID"].Value.ToString();

            if (Application.OpenForms["fr_" + filtre_ID] == null)
            {
                Fr_Foto_Galeri f = new Fr_Foto_Galeri(tablo_adi, filtre_ID, adi);
                f.MdiParent = Application.OpenForms["fr_ana"];

                f.Show();
            }
            else
            {
                Application.OpenForms["fr_" + filtre_ID].Focus();
            }
